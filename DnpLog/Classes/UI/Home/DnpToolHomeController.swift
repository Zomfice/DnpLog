...
class DnpToolHomeController: DnpToolBaseController {
    ...
    func jumpto(indexPath: IndexPath) {
        ...
        /*
         use_frameworks! 动态库, 使用DnpToolCommon.self获取Bundle是DnpTool的包名
         use_frameworks! :linkage => :static DnpToolCommon.self静态库获取的Bundle是DnpTool-Example主工程包名
         */
        // use_frameworks! :linkage => :static +  use_frameworks!
        
        if homeModel.jump.count > 0,let clsName = Bundle(for: DnpToolCommon.self).infoDictionary!["CFBundleName"] as? String{
...
internal class DnpHomeCollectCell: UICollectionViewCell {
    ...
    lazy var icon: UIImageView = {
        ...
        m_icon.layer.cornerRadius = 30
        m_icon.layer.masksToBounds = true
        return m_icon
    }()
    ...
    func setModel(model: DnpHomeModel){
        ...
        self.icon.image = UIImage.imageName(name: model.icon) ?? randomColor.image
    }
